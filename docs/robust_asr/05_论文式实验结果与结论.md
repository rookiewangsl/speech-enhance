...
3. LoRA 的基础收益同时包含普通话/AISHELL 任务适配与中文数字输出风格适配；数字规范化后仍有显著
   剩余收益。MCT 在 Raw 上有独立收益，但在
...
### 2.9 LoRA 兼有任务与输出风格适配，MCT 在 Raw 上还有独立收益
...
**结论 9a。** W0→Clean-LoRA 将主 clean CER 从 `14.08%` 降到 `7.76%`；固定数字规范化后
是 `10.95%→7.76%`，差值 `−3.18 pp`，95% CI=`[−3.75,−2.62] pp`。这证明 W0→MCT 的
大幅改善不能全部归因于混响增强，也不能全部称为声学/语言能力提升：Clean-LoRA 同时学习了普通话、
AISHELL 录音域、任务形式和中文数字书写风格。Clean-LoRA 仍是分离混响增强贡献所必需的对照组。
...
**结论 11。** Paraformer clean CER 为 `2.33%`，明显强于 Whisper W0 的主 CER `14.08%`；即使采用
固定数字规范化，W0 仍为 `10.95%`。同时
...
| 模型/条件 | 预注册主 CER | 固定数字规范化 CER | 逐句读法诊断下界 |
|---|---:|---:|---:|
| W0 Clean | 14.08% | 10.95% | 10.60% |
| W0 Robust Raw | 23.61% | 20.82% | 20.48% |
| W0 Robust M-WPE | 15.39% | 12.47% | 12.12% |
| Clean-LoRA Clean | 7.76% | 7.76% | 7.76% |
| MCT-LoRA Clean | 7.83% | 7.83% | 7.83% |
| Paraformer Clean | 2.33% | 2.33% | 2.33% |

固定规则不根据参考答案逐句选读法；全部逐位读取的敏感性结果在 W0 Clean/Robust Raw/Robust
M-WPE 上分别为 `11.39%/21.21%/12.86%`，与固定主辅助规则相差不超过 `0.45 pp`。

W0 的 1,000 条 clean 中有 141 条 hypothesis 含阿拉伯数字，136 条经固定规则改善、2 条恶化。
这些数字输出样本的 CER 从 `31.82%` 降至 `11.59%`，无数字输出样本保持 `10.83%`。
因此 W0 的部分绝对错误是“四点八”与“4.8”等书写形式差异，而非声学失败。

数字规范化也修正了 LoRA 的解释：W0→Clean-LoRA 的 clean 改善从主指标 `−6.32 pp`
变为 `−3.18 pp`，95% CI `[−3.75,−2.62] pp`；约 `3.14 pp` 的表观差异与数字书写形式有关，
剩余收益仍显著。相反，W0 的 Robust WPE 收益为 `−8.35 pp`
（`[−9.16,−7.65] pp`），MCT 相对 Clean-LoRA 的 Raw 收益仍为 `−1.16 pp`
（`[−1.42,−0.91] pp`），核心混响结论未改变。主 CER 保持为预注册指标；固定数字规范化 CER
只作为辅助分析。完整对照见 `06_数字规范化CER对照实验.md`。

## 4. 结论综合

### 4.1 方法之间的关系

1. **普通话/AISHELL 任务适配与 WPE 互补。** W0+WPE 的 Robust CER 为 `15.39%`，几乎等于
...
| 只追求当前普通话绝对 CER | Paraformer 作为更强基线 | 数字规范化 clean `2.33%` vs Whisper W0 `10.95%` |
